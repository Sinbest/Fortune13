...
/turf/closed/indestructible/f13/matrix/MouseDrop_T(atom/dropping, mob/user)
	...
	if(departing_mob != user && departing_mob.client)
		...
		return
	if(alert("Are you sure you want to [departing_mob == user ? "depart the area for good (you" : "send this person away (they"] will be removed from the current round, the job slot freed)?", "Departing West Virginia", "Confirm", "Cancel") != "Confirm")
		return
	...
	if(departing_mob.stat == DEAD)
		...
		departing_mob.visible_message("<span class='notice'>[departing_mob == user ? "Out of their own volition, " : "Ushered by [user], "][departing_mob] crosses the border and departs West Virginia.</span>")
